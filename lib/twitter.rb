...
# def spam
#   $A.sample(5).each do |user|  # B << "@JordanLaforet"
#   # for i in B
#      login_twitter().update('Bonjour' + ' @' + user + "! Wow merci pour votre travail minutieux d'investigation, d'analyse et de restitution de l'information, vous faites un travail magnifique ! Continuez comme ça :) #bonjour_monde @the_hacking_pro ")
#    end
#  end
 # spam
...
#    login_twitter().follow(user_id=tweet["user"]["id"])
#  end
# end
#
#  follow
...
def stream ()
  client=login_twitter()
  client.filter(track:"#bonjour_monde").each do |object|
